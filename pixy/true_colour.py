...
class TrueColour(EscapeSequence):
    ...
    def __new__(cls, red: int, green: int, blue: int, background: bool = False) -> "TrueColour":

        if min(red, green, blue) < 0 or max(red, green, blue) > cls.__MAX_VALUE:

            raise ValueError(
                "RGB values must be in-between 0 and %d" % cls.__MAX_VALUE
            )
        ...
        return cast(TrueColour, super().__new__(cls, type_code, cls.__RGB_SUB_CODE, red, green, blue))
